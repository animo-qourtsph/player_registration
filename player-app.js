(() => {
  ...
  function submitRegistration(){
    ...
    state.registrationType="pair";
    state.status=paymentHeld?'Pending Level Validation':
    ...
    triggerRegistrationSubmissionEmails(record);
  ...
  function renderConfirmation(){
    ...
    if(message){
      const emails=[state.player1?.email,state.player2?.email].filter(Boolean);
      message.textContent=emails.length>=2
        ?"A registration-received email is triggered for Player 1 and Player 2. We’ll also email both players whenever there is an important update to this team registration."
        :"Registration email updates are automatic for every player with an email address on file.";
    }
  }
  ...
  function lookup(event){
    ...
    if(!record){
      out.innerHTML=`<div class="lookup-error">We couldn't verify that registration. Check the reference and the email or mobile used during registration.</div>`;
      return;
    }
    out.innerHTML=lookupResultMarkup(record);
