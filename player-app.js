(() => {
  ...
  const STORAGE_KEY = `animo-registration-draft-${config.tournamentId}-v21`;
  ...
  function renderLanding(){
    ...
    const snapshot = [
      ["Confirmed",getConfirmedParticipantCount().toLocaleString("en-PH"),"fomo"],
    ...
    const rules=[
      ...
      {icon:"N",title:"No DUPR?",copy:"Request your level and provide your club page + playing history."},
  ...
  function renderPayment(){
    ...
    return `${stepHeading("04 · Payment","Review the fee before paying.","Charges stay visible before submission. Payment only becomes verified after organizer review.")}
  ...
  function validateStep(){
    ...
    if(state.step===3){/* Payment is intentionally skipped while level/division verification is pending. */}
  ...
  function playerEmailWorkflowEnabled(templateId){
    try{
      const settings=JSON.parse(localStorage.getItem(EMAIL_NOTIFICATION_KEY)||"null");
      if(!settings)return true;
      if(settings.enabled===false)return false;
      const template=Array.isArray(settings.templates)?settings.templates.find(t=>t.id===templateId):null;
      return template?template.enabled!==false:true;
    }catch{return true}
  }

  function playerEmailName(player){
    return [player?.firstName,player?.middleName,player?.lastName].filter(Boolean).join(" ").trim()||"Player";
  }

  function recordPlayerEmailTrigger(record,templateId,recipients,source){
    const activity=playerEmailActivity();
    const enabled=playerEmailWorkflowEnabled(templateId);
    const labels={
    ...
    recipients.forEach(({slot,player})=>{
      const email=String(player?.email||"").trim().toLowerCase();
      let result="Triggered";
      let detail="Automatic trigger recorded. Actual delivery starts after backend integration.";
      if(!enabled){
        result="Suppressed";
        detail="This email notification is currently disabled.";
      }else if(!email){
        result="Skipped";
        detail=`${slot} does not have an email address on file.`;
      }
      ...
      activity.unshift({
        ...
        templateId,
        templateLabel:labels[templateId]||templateId,
        recipientName:playerEmailName(player),
    ...
    try{localStorage.setItem(EMAIL_ACTIVITY_KEY,JSON.stringify(activity.slice(0,250)))}catch(e){}
  }

  function triggerRegistrationSubmissionEmails(record){
    const primary=[
      {slot:"Player 1",player:record.player1},
      {slot:"Player 2",player:record.player2}
    ];
    recordPlayerEmailTrigger(record,"registration_received",primary,"Team registration was submitted");
  }
  ...
  async function submitRegistration(){
    ...
    try{
      ...
      state.status=record.status||"Pending Level Validation";
      state.submitted=true;
      state.registrationType="pair";

      // Keep only a local convenience cache; Supabase is now the source of truth.
      try{
        const cached={...structuredClone(record),lookupIdentity:{
          email:(state.player1.email||"").toLowerCase(),
          mobile:normalizeMobile(state.player1.mobile)
        }};
        saveRecord(cached);
      }catch(e){console.warn("Local convenience cache could not be saved",e)}
  ...
  function renderConfirmation(){
    ...
    if(message){
      ...
      if(delivery?.sent>=2){
        message.textContent="Registration confirmation emails were sent to Player 1 and Player 2. Keep your registration reference for future status checks.";
      }else if(delivery?.sent===1){
        message.textContent="Your registration was saved and one confirmation email was sent. The second email could not be delivered.";
        ...
        message.textContent="Your registration is safely stored, but the confirmation emails could not be delivered. Your registration reference is still valid.";
      }else{
  ...
  function lookupDivisionName(record){
    ...
    if(record?.status==="Awaiting Partner")return "Division pending partner completion";
  ...
  function lookupVerificationLabel(record){
    ...
    if(status==="Pending Level Validation"||status==="Pending Level Verification")return "Organizer validation pending";
    if(status==="Awaiting Partner")return "Waiting for partner";
    ...
    if(v1.verified&&(!v2||v2.verified))return "DUPR proof verified";
    return "Under review";
  }

  function lookupPaymentLabel(record){
    if(record?.paymentHeld)return "On hold until placement is finalized";
    if(record?.payment?.fileName)return "Payment proof submitted";
    if(record?.status==="Confirmed"||record?.status==="Approved")return "Registration accepted";
  ...
  function lookupStatusModel(record){
    ...
    const base={
      ...
      headline:"Animo is reviewing your registration.",
      message:"Your registration is in the review queue. The organizer is checking the information you submitted before final approval.",
      nextTitle:"No action needed for now",
      nextBody:"Keep your registration reference. We’ll update this page as soon as your registration status changes.",
      progress:2
    };

    if(status==="Submitted"){
      return {...base,
        visibleStatus:"Received",
        headline:"Animo received your registration.",
        message:"Your entry has been received and is now waiting for organizer review.",
        nextTitle:"What happens next?",
        nextBody:"The organizer will review your player details, level information, and division placement before approval."
      };
    }

    if(status==="Payment Submitted"){
      return {...base,
        visibleStatus:"Review in Progress",
        headline:"Animo is reviewing your registration.",
        message:"We received your registration and payment proof. The organizer is checking your submitted details before approval.",
        nextBody:"No action is needed unless the organizer contacts you for clarification or updated proof."
      };
    }

    if(status==="Pending Level Validation"||status==="Pending Level Verification"){
      return {...base,
        tone:"verification",
        visibleStatus:"Level Verification",
        eyebrow:"Fair-play verification",
        headline:"Animo is verifying your playing level.",
        message:"We’re reviewing your DUPR proof or the club and playing-history information you provided. This helps keep every division fair.",
        nextTitle:"No action needed for now",
        nextBody:"The organizer will approve the submitted level or contact you if more information is needed."
      };
    }
    ...
    if(status==="Awaiting Partner"){
      return {...base,
        tone:"waiting",
        visibleStatus:"Waiting for Partner",
        eyebrow:"Partner completion required",
        headline:"Waiting for your partner.",
        message:"Your registration is saved, but the final level, doubles category, and division cannot be completed until your partner finishes their details.",
        nextTitle:"Your partner needs to complete their entry",
        nextBody:"Once your partner is linked and their player information is complete, Animo can continue the registration review.",
        progress:1
      };
    }
    ...
    if(status==="Pending"){
      return {...base,
        visibleStatus:"Final Review",
        headline:"Your registration is under review.",
        message:"The tournament team is completing the final validation of your player details and tournament entry.",
        nextBody:"No action is required unless the organizer contacts you."
      };
    }
    ...
    if(status==="Approved"){
      return {...base,
        ...
        eyebrow:"Registration accepted",
        headline:"Your registration has been accepted.",
        message:"Your player and division details have been approved. The organizer is completing the final confirmation of your tournament entry.",
        nextTitle:"Almost there",
        nextBody:"Your entry has passed registration review. Watch this page for final tournament confirmation.",
        progress:3
      };
    }
    ...
    if(status==="Confirmed"){
      return {...base,
        ...
        message:`See you on ${eventDate} at ${venue}. Your tournament entry is confirmed.`,
        nextTitle:"You’re all set",
        nextBody:"Keep your registration reference and watch for official schedules, player instructions, and tournament-day announcements.",
        progress:4
      };
    }
    ...
    if(status==="Rejected"){
      return {...base,
        tone:"rejected",
        visibleStatus:"Not Approved",
        eyebrow:"Registration decision",
        headline:"Your registration was not approved.",
        message:"The organizer could not approve this tournament entry based on the information currently on file.",
        nextTitle:"Need clarification?",
        nextBody:"Please contact the tournament organizer if you believe your registration should be reviewed again.",
        progress:2,
        stopped:true
      };
    }
  ...
  function lookupProgressMarkup(model){
    const steps=[
      ["Registration Received","We have your entry."],
      ["Verification","Player and level review."],
      ["Approved","Entry accepted."],
      ["Confirmed","Official tournament entry."]
    ];
  ...
  function lookupResultMarkup(record){
    ...
    return `<div class="lookup-status-dashboard lookup-tone-${esc(model.tone)}">
      <section class="lookup-status-hero">
        <div class="lookup-status-icon" aria-hidden="true">${model.tone==="confirmed"?"✓":model.tone==="approved"?"✓":model.tone==="rejected"?"!":model.tone==="cancelled"?"×":model.tone==="waiting"?"…":"⌁"}</div>
      ...
      <section class="lookup-next-card">
        ...
        <div>
          ...
          ${contact&&["Rejected","Cancelled"].includes(model.raw)?`<small>Organizer contact: ${esc(contact)}</small>`:""}
        </div>
      </section>
  ...
  async function refreshTrackerSilently(){
    ...
    try{
      ...
      if(document.body.dataset.activeView==="confirmation"){
        ...
        state.status=response.record.status||state.status;
        $("#confirmationStatus").textContent=state.status;
        if(previous!==state.status){
          showToast(`Registration status updated: ${lookupStatusModel(response.record).visibleStatus}`);
        }
      }
    }catch(error){
  ...
  function startTrackerRefresh(){
    ...
    trackerRefreshTimer=setInterval(refreshTrackerSilently,15000);
  }
  ...
  function handleAction(a){
    ...
    if(a==='register'){state.registrationType='pair';state.step=Math.max(0,Math.min(state.step,stepDefs.length-1));showView('registration');renderStep()}
  ...
  loadAdminConfig();loadDraft();renderLanding();document.body.dataset.activeView="landing";bindGlobal();
})();
