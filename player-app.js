(() => {
  ...
  function triggerRegistrationSubmissionEmails(record){
    const primary=[{slot:"Player 1",player:record.player1}];
    if(record.registrationType==="pair")primary.push({slot:"Player 2",player:record.player2});
    recordPlayerEmailTrigger(record,"registration_received",primary,"Registration was submitted");

    if(record.registrationType==="invite"){
      recordPlayerEmailTrigger(record,"partner_invitation",[{slot:"Player 2",player:record.player2}],"Partner invitation was created");
    }
  }
  ...
  function renderConfirmation(){
    ...
    if(message){
      const recipients=[state.player1?.email,state.registrationType==="pair"?state.player2?.email:null].filter(Boolean);
      if(state.registrationType==="invite"){
        message.textContent="A registration-received email is triggered for Player 1, and the partner invitation is triggered for Player 2. We’ll continue emailing players when important registration statuses change.";
      }else if(recipients.length>=2){
        message.textContent="A registration-received email is triggered for Player 1 and Player 2. We’ll also email both players when important registration statuses change.";
      }else{
        message.textContent="A registration-received email is triggered automatically. Future important status changes will also generate email notifications for players with an email address on file.";
      }
    }
  }
