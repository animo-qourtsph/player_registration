(() => {
  ...
  const EXPECTED_API_CONTRACT = "2026.09.10.1";
  ...
  function applyPortalState(response){
    ...
    if(Number.isFinite(Number(tournament.confirmedParticipants))){
      config.publicStats.confirmedParticipantsOverride=Math.max(0,Number(tournament.confirmedParticipants));
    }
    ...
    config.registrationActive=tournament.active!==false;
  ...
  async function ensureProductionBackend({refreshPortal=false}={}){
    ...
    const stale=!portalStateLoadedAt||(Date.now()-portalStateLoadedAt)>60000;
  ...
  function renderPayment(){
    ...
    const paymentMethodsStale=!livePaymentMethodsLoadedAt||(Date.now()-livePaymentMethodsLoadedAt)>600000;
  ...
  function paymentDetails(m){
    ...
    return `<div class="subsection payment-method-details">
      ...
      <div class="form-grid">
        ${field("payment","reference","Payment Reference Number",state.payment.reference,"text","off",false)}
        ${field("payment","senderName","Account / Sender Name",state.payment.senderName,"text","name",false)}
        ${field("payment","amountPaid","Amount Paid",state.payment.amountPaid,"number","off",false)}
        ${field("payment","paymentDate","Payment Date",state.payment.paymentDate,"date","off",false)}
      </div>
      ...
      <div class="file-drop" style="margin-top:18px">
        ...
        ${state.payment.fileName?`<div class="file-confirm">✓ ${esc(state.payment.fileName)}</div>`:""}
      </div>
      <p class="submit-note">Uploading proof sets the registration to <strong>Payment Submitted</strong>, not Payment Verified.</p>
    </div>`;
  }
